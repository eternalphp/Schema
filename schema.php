...
$control->create("project",function($table){
	...
	$table->string("description",200)->nullable()->comment("项目介绍");
	$table->integer("userid")->alias("creater")->comment("创建人");
	$table->datetime("createtime")->comment("创建时间");
...
$control->create("examine",function($table){
	...
	$table->string("pcode",20)->nullable()->comment("问卷编号");
	$table->integer("projectid")->comment("所属项目");
	$table->integer("examineType")->comment("问卷类型，1：调查问卷，2:在线试卷");
...
$control->create("examine_reward_rule",function($table){
	...
	$table->string("description")->nullable()->comment("奖励说明");
...
$control->create("question",function($table){
	...
	$table->string("description",200)->nullable()->comment("问题说明");
...
/* 	
	...
	$table->string("answerItems",200)->nullable()->comment("问题答案");
...
$control->create("question_rules_mutexs",function($table){
	...
	$table->integer("mutexIndex")->nullable()->comment("排斥选项序号");
	$table->integer("mutexItemIndex")->nullable()->comment("排斥选项");
...
$control->create("examine_users",function($table){
	...
	$table->string("deviceID",50)->nullable()->comment("设备ID");
	$table->string("ucode",2)->nullable()->comment("ucode");
	...
	$table->string("device",50)->nullable()->comment("用户设备");
	$table->string("browser",50)->nullable()->comment("浏览器");
	$table->string("redirect_uri",200)->nullable()->comment("回调地址");
	$table->text("answer")->nullable()->comment("答案");
	$table->datetime("completetime")->nullable()->comment("完成时间");
	$table->datetime("createtime")->comment("创建时间");
	$table->tinyInt("status")->nullable()->comment("完成状态");
	$table->integer("creater")->nullable()->comment("创建人");
...
//定时任务
$control->create("crontab",function($table){
	$table->integer("id")->unsigned()->increments();
	$table->integer("noticeid")->comment("noticeid");
	$table->integer("userid")->comment("userid");
	$table->tinyInt("status")->defaultVal(0)->comment("状态，0：未推送，1：已推送");
	$table->datetime("createtime")->comment("创建时间");
	$table->timestamp("updatetime")->comment("更新时间");
	$table->comment("定时任务");
});
...
//手机验证码
$control->create("verification",function($table){
	$table->integer("id")->unsigned()->increments();
	$table->char("mobile",11)->comment("手机号");
	$table->integer("code")->comment("code");
	$table->tinyInt("status")->defaultVal(0)->comment("使用状态，0：未使用，1：已使用");
	$table->datetime("createtime")->comment("创建时间");
	$table->comment("手机验证码");
});
